...
fn parse_string(content: String) -> Result<Input, Box<dyn Error>> {
    ...
    let draw_numbers = lines[0].split(',').map(|number| number.parse::<u8>()).collect::<Result<Vec<u8>, _>>()?;
    ...
    for n in 0..board_count {
        for i in 0..5 {
            // Parse the line
            let ref mut line_numbers = lines[2 + n * 6 + i].split_ascii_whitespace().map(|number| number.parse::<u8>()).collect::<Result<Vec<u8>, _>>()?;
